...
from computer import existing_browser as _existing_browser
...
class _BrowserSession:
    ...
    def __init__(self, browser_name: str):
        ...
        # True when this session drives the browser the user already had open.
        # An attached browser is theirs: Jarvis must never close it.
        self._attached = False
        # Why this session got the profile it did, when that is worth saying
        # out loud (signed-out second profile, or a fallback). Reported once.
        self._profile_note = ""
    ...
    async def _async_close(self):
        if self._context:
            try:
                if self._attached:
                    # The context is the user's own browser. Disconnect from it
                    # instead of closing it: "close the browser" must not shut
                    # down the Chrome window they are working in.
                    print("[Browser] Detaching (this browser belongs to the user)")
                else:
    ...
    async def _adopt_attached_page(self) -> Page:
        """The tab to drive inside an attached browser.

        A browser that is already open has the user's tabs; opening a new one
        would leave a stray tab in their session, so the most recent tab is
        reused and navigated, exactly like before.
        """
        await asyncio.sleep(0.2)
        pages = self._context.pages
        return pages[-1] if pages else await self._context.new_page()

    def take_profile_note(self) -> str:
        """The one-off explanation of which profile this session got, if any.

        Consumed once so a multi-step task states the reason a single time
        instead of repeating it on every action.
        """
        note, self._profile_note = self._profile_note, ""
        return note
    ...
    async def _launch(self):
        ...
        # Can Jarvis drive the browser the user already has open? Only an
        # attach over the DevTools endpoint reaches their signed-in session;
        # a second launch is a different, signed-out browser. This is the
        # difference between clicking Gmail's Compose button and staring at a
        # login page.
        info = _existing_browser.resolve(self.browser_name)

        if info["how"] == _existing_browser.ATTACH:
            try:
                self._context = await engine_obj.connect_over_cdp(info["cdp_url"])
                self._page = await self._adopt_attached_page()
                self._attached = True
                self._profile_note = info["note"]
                print(f"[Browser] ✅ Attached [{label}] via CDP {info['cdp_url']}")
                return
            except Exception as e:
                print(f"[Browser] ⚠️  CDP attach failed ({e}); falling back to a launch")

        if info["how"] == _existing_browser.SECOND_PROFILE:
            # Their browser holds the only lock on the real profile. Keep the
            # reason so the caller can say so, rather than presenting a
            # signed-out window as the user's own session.
            self._profile_note = info["note"]
        ...
        try:
            ...
            if not self._profile_note:
                self._profile_note = (
                    f"your real {self.browser_name} profile could not be opened "
                    f"({type(e).__name__}), so this is a separate JARVIS profile; "
                    f"pages that need a sign-in will not be logged in"
                )
        ...
        if not self._profile_note:
            self._profile_note = (
                f"your real {self.browser_name} profile could not be opened, so "
                f"this is a separate JARVIS profile; pages that need a sign-in "
                f"will not be logged in"
            )
        ...
        try:
            ...
            # The reason this session got the profile it did is already known
            # (``self._profile_note``) and says more than the launch error alone.
            # It is attached as well as written into the message, so the facade can
            # carry it into its diagnostic rather than truncating it away — losing
            # this rich explanation is why a correct diagnosis arrived as a generic
            # "driver error".
            err = RuntimeError(
                f"Could not launch {self.browser_name}: {e2}"
                + (f" — {self._profile_note}" if self._profile_note else "")
            )
            err.profile_note = self._profile_note
            raise err from e2
...
class _SessionRegistry:
    ...
    def native_url(self) -> str:
        """Last natively opened URL without consuming it — for verification."""
        return self._last_native_url or ""
...
def browser_control(
    ...
    # State once, honestly, which browser this really is: a signed-out second
    # profile explains a missing button far better than a bare failure.
    try:
        note = sess.take_profile_note()
    except Exception:
        note = ""
    if note:
        result = f"{result}\n[profile] {note}"
