...
pyperclip = lazy_module("pyperclip")
...
def _smart_type(text: str, clear_first: bool = True) -> str:
    ...
    if len(text) > 20 and bool(pyperclip):
        pyperclip.copy(text)
        time.sleep(0.1)
...
def _click(x=None, y=None, button: str = "left", clicks: int = 1) -> str:
    ...
    if x is not None and y is not None:
        ...
        # Show JARVIS blue dot cursor at click position
        try:
            from core.jarvis_cursor import show_jarvis_click
            show_jarvis_click(int(x), int(y))
        except Exception:
            pass
...
def _move(x: int, y: int, duration: float = 0.3) -> str:
    ...
    # Show JARVIS blue dot cursor at target position
    try:
        from core.jarvis_cursor import show_jarvis_move
        show_jarvis_move(int(x), int(y))
    except Exception:
        pass
    return f"Mouse → ({x}, {y})"
...
def _drag(x1: int, y1: int, x2: int, y2: int, duration: float = 0.5) -> str:
    ...
    # Show JARVIS blue dot cursor at drag end position
    try:
        from core.jarvis_cursor import show_jarvis_click
        show_jarvis_click(int(x2), int(y2))
    except Exception:
        pass
