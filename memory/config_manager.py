...
def get_fish_audio_api_key() -> str | None:
    """Return the Fish Audio API key without exposing it in logs or UI text."""
    return load_api_keys().get("fish_audio_api_key")


def save_fish_audio_config(
    api_key: str,
    model_id: str = "s1",
    voice_id: str = "",
) -> None:
    """Persist Fish Audio credentials and the selected model/reference voice."""
    _patch_config(
        fish_audio_api_key=(api_key or "").strip(),
        fish_audio_model_id=(model_id or "").strip(),
        fish_audio_voice_id=(voice_id or "").strip(),
    )
...
def get_voice() -> str:
    ...
    Provider-scoped: the Gemini provider's own voice (voice.providers.gemini)
    is read first; the legacy flat ``voice_name`` remains the fallback so
    pre-migration configs keep working. A save for any OTHER provider can no
    longer touch this value.
    """
    data = load_api_keys()
    gem_voice = ""
    try:
        vblock = data.get("voice") or {}
        providers = vblock.get("providers") or {}
        gem_voice = str((providers.get("gemini") or {}).get("voice") or "").strip()
    except Exception:
        gem_voice = ""
    v = gem_voice or data.get("voice_name", DEFAULT_VOICE) or DEFAULT_VOICE
    return v if v in AVAILABLE_VOICES else DEFAULT_VOICE


def save_voice(voice_name: str) -> None:
    """Persist the chosen Live voice into the Gemini provider's own subtree
    (plus the legacy flat key, mirrored in the same write, so dual-read
    binaries keep working). Unknown names collapse to the default so a bad
    value can never reach the API and break the session."""
    ensure_config_dir()
    data: dict = {}
    if CONFIG_FILE.exists():
        try:
    ...
    v = v if v in AVAILABLE_VOICES else DEFAULT_VOICE
    data["voice_name"] = v
    try:
        vblock = dict(data.get("voice") or {})
        providers = dict(vblock.get("providers") or {})
        gem = dict(providers.get("gemini") or {})
        gem["voice"] = v
        providers["gemini"] = gem
        vblock["providers"] = providers
        data["voice"] = vblock
    except Exception:
        pass
    CONFIG_FILE.write_text(json.dumps(data, indent=4), encoding="utf-8")
