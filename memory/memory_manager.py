...
_lock            = Lock()
_memory_cache: dict = {}   # {"rev": (mtime_ns, size), "data": parsed dict}
...
def load_memory() -> dict:
    ...
    with _lock:
        try:
            # mtime+size gate: parse once, re-read only when the store changes.
            # Reconnect/prompt paths hit this repeatedly; hitting disk every time
            # was pure waste since writers always update the file's mtime.
            try:
                st = MEMORY_PATH.stat()
                _rev = (st.st_mtime_ns, st.st_size)
            except OSError:
                _rev = None
            if _rev is not None and _memory_cache.get("rev") == _rev:
                cached = _memory_cache.get("data")
                if isinstance(cached, dict):
                    # Hand back fresh top-level + category dicts so a caller
                    # mutating its copy can never corrupt the cached snapshot.
...
# Classic store category → Brain graph category. The Brain categories drive
# which orbital ring a memory sits on in the 3D orb, so each classic fact
# lands on a real ring instead of a random position.
_BRAIN_CATEGORY = {
    "identity":      "PROFILE",
...
def _brain_write_through(memory_update: dict) -> None:
    """Mirror newly saved facts into the Brain SQLite store.

    One store feeds the prompt-adjacent ranking/recall, the other the 3D orb
    and the tools; write-through keeps them a single source of truth instead
    of two drifting archives. Best-effort by design: a locked or missing Brain
    store must never fail a memory save (spec §20).
    """
    try:
        ...
        for cat, items in memory_update.items():
            ...
            for key, entry in items.items():
                ...
                if not value:
                    continue
                brain.remember(
                    content=f"{_pretty(key)}: {value}",
                    category=_BRAIN_CATEGORY.get(str(cat).lower(), "CONTEXT"),
                    source="explicit",
                    importance=0.7,
                    confidence=0.95,   # explicitly saved by/about the user
                )
    except Exception as exc:
        print(f"[Memory] brain write-through skipped: {exc}")
...
def update_memory(memory_update: dict) -> dict:
    ...
    if _recursive_update(memory, memory_update):
        save_memory(memory)
        _brain_write_through(memory_update)
        # ASCII-safe: a cp1252 Windows console would crash on the emoji.
        print(f"[Memory] Saved: {list(memory_update.keys())}")
    return memory

def _orb_event(kind: str, count: int = 1) -> None:
    """Feed a live memory event to the 3D orb (any thread, best-effort).
    The orb module imports Qt lazily; a headless/test environment without it
    simply skips the visual side-effect."""
    try:
        from dashboard.brain3d import emit_memory_event
        emit_memory_event(kind, count)
    except Exception:
        pass
...
def format_memory_for_prompt(memory: dict | None) -> str:
    ...
    for field in _IDENTITY_FIELDS:
        ...
        if not val:
            continue
        if field == "language":
            # Past observation only — JARVIS always answers in English.
            core_lines.append(
                f"Has spoken to you in: {val} (past observation — "
                f"always answer in English regardless)")
        else:
...
def search_memory(query: str, limit: int = 8) -> str:
    ...
    result = head + "\n" + "\n".join(lines) + more
    _orb_event("recall", count=max(1, min(len(rows), 4)))

    # Merge ranked hits from the Brain store (importance/confidence/recency
    # weighted) so recall sees BOTH archives, not just the JSON one.
    try:
        from memory.manager import get_brain_memory
        brain = get_brain_memory()
        if brain is not None:
            hits = brain.retrieve(query or "", limit=max(0, limit - len(lines)))
            if hits:
                extra = "\n".join(
                    f"brain/{h.get('category', '?').lower()}: {h.get('content', '')}"
                    f" [{float(h.get('confidence', 0)):.2f}]" for h in hits
                )
                result += "\n" + extra
    except Exception:
        pass
...
def forget(key: str, category: str = "notes") -> str:
    ...
    if key in cat:
        ...
        memory[category] = cat
        save_memory(memory)
        # Keep the Brain store in sync: forget must mean forget in BOTH stores,
        # or the 3D orb keeps showing a memory JARVIS no longer uses.
        try:
            from memory.manager import get_brain_memory
            brain = get_brain_memory()
            if brain is not None:
                target = f"{_pretty(key)}:"
                for m in brain._db.iterate_active():
                    if str(m.get("category", "")).lower() == category.lower() \
                            and str(m.get("content", "")).startswith(target):
                        brain._db.delete_memory(int(m["id"]))
        except Exception as exc:
            print(f"[Memory] brain forget skipped: {exc}")
        _orb_event("forget")
