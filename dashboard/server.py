...
def _ensure_crypto_js() -> None:
    """Fetch CryptoJS once and cache it locally.

    NOTE: this performs a NETWORK download. It used to run at module import
    time, which silently froze JARVIS startup for seconds (or much longer on
    machines with slow/blocked internet) before the AI could connect. It is
    now invoked from serve() — still once per process, but off the startup
    path and only when the dashboard actually starts serving.
    """
...
<p>Connecting to JARVIS…</p>
    ...
    async def serve(self) -> None:
        ...
        # CryptoJS cache is a network download — run it in the background so it
        # can never delay the server (or the rest of JARVIS) coming up.
        asyncio.get_event_loop().run_in_executor(None, _ensure_crypto_js)
