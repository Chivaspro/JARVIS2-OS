...
GEMINI_MODEL       = "default"   # model-router role (core/model_router.py)
...
def _get_gemini(model: str = GEMINI_MODEL):
    ...
    class _W:
        def generate_content(self, contents):
            if model in model_router.ROLES:
                return model_router.generate(_c, contents=contents, role=model)
...
Be specific and actionable. If you see an error message, quote it exactly."""
        ...
        from core import model_router
        response = model_router.generate(client, contents=contents, role="vision")
